...
class DashboardWindow(QWidget):
    ...
    def _build_sidebar(self) -> QFrame:
        ...
        for label, index in destinations:
    ...
    def _build_dashboard_page(self) -> QWidget:
        ...
        row.addWidget(self.expenses_card)
        row.addWidget(self.risk_card)
    ...
    def _build_accounts_page(self) -> QWidget:
        ...
        layout.addWidget(QLabel("Double-cliquez sur une ligne pour ouvrir l'historique."))
        self.accounts_table = QTableWidget(0, 6)
        self.accounts_table.setHorizontalHeaderLabels(
            ["Compte", "Type", "Solde", "Portée", "Établissement", "Actions"]
        )
    ...
    def _build_analysis_page(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        title = QLabel("Analyses & projections")
        title.setObjectName("Title")
        layout.addWidget(title)
        self.analysis_label = QLabel("—")
        self.analysis_label.setWordWrap(True)
        layout.addWidget(self.analysis_label)
        self.forecast_chart = LineChart([])
        layout.addWidget(self.forecast_chart, 1)
        return page

    def _build_budget_page(self) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        title = QLabel("Budget du foyer")
        title.setObjectName("Title")
        layout.addWidget(title)
        self.budget_label = QLabel("—")
        self.budget_label.setWordWrap(True)
        layout.addWidget(self.budget_label)
        edit = QPushButton("Modifier le budget")
    ...
    def toggle_drawer(self) -> None:
        ...
        title = QLabel("Personnaliser l'interface")
        title.setObjectName("Title")
        layout.addWidget(title)
        layout.addWidget(QLabel("Apparence"))

        theme = QComboBox()
        theme.addItems(["Sombre", "Claire"])
        theme.setCurrentIndex(0 if self._preferences["theme"] == "dark" else 1)
        layout.addWidget(theme)

        accent = QLineEdit(str(self._preferences["accent_color"]))
        layout.addWidget(QLabel("Couleur d'accent"))
        layout.addWidget(accent)

        compact = QCheckBox("Mode compact")
        compact.setChecked(bool(self._preferences["compact_mode"]))
        layout.addWidget(compact)

        options = []
        preference_options = [
            ("show_net_worth", "Graphique patrimoine"),
            ("show_accounts", "Comptes"),
            ("show_budget", "Budget"),
            ("show_analytics", "Analyses"),
        ]
        for key, label in preference_options:
        ...
        layout.addStretch()
        save.clicked.connect(lambda: self._save_preferences(theme, accent, compact, options))
    ...
    def _save_preferences(self, theme, accent, compact, options) -> None:
        ...
        if not color.startswith("#") or len(color) not in (4, 7):
            QMessageBox.warning(
                self,
                "Paramètres",
                "Couleur invalide. Exemple : #6C63FF.",
            )
            return

        self._preferences.update(
            {
                "theme": "dark" if theme.currentIndex() == 0 else "light",
                "accent_color": color,
                "compact_mode": compact.isChecked(),
                "show_net_worth": options[0].isChecked(),
                "show_accounts": options[1].isChecked(),
                "show_budget": options[2].isChecked(),
                "show_analytics": options[3].isChecked(),
            }
        )
    ...
    def refresh(self) -> None:
        ...
        expense_text = "\n".join(
            f"• {key}: {money(value)}" for key, value in list(expenses.items())[:6]
        )
        self.expenses_card._content_label.setText(  # type: ignore[attr-defined]
            expense_text or "Aucune dépense enregistrée."
        )
        self.risk_card._content_label.setText(  # type: ignore[attr-defined]
            f"HHI {float(data['hhi']):.2f} · {len(data['allocation_type'])} classes d'actifs. "
            "Indicateur descriptif, pas un conseil d'investissement."
        )

        history = self._finance.net_worth_history(self._user.id)
        self.net_chart.set_points(
            [(str(row["date"]), Decimal(row["net"])) for row in history]
        )
        forecast = data["forecast"]
        self.forecast_chart.set_points(
            [(f"An {row.year}", Decimal(row.value)) for row in forecast]
        )
        self.analysis_label.setText(
            f"Rendement pondéré renseigné : {float(data['expected_return']) * 100:.2f} %. "
            f"Projection centrale à 10 ans : {money(forecast[-1].value)}. "
            "Les scénarios dépendent des hypothèses saisies."
        )
        self.budget_label.setText(
            f"Dépensé : {money(data['budget_spent'])}\n"
            f"Budget prévu : {money(data['budget_planned'])}"
        )
    ...
    def _refresh_accounts(self, accounts) -> None:
        ...
        for row, account in enumerate(accounts):
            ...
            callbacks = [
                ("Modifier", lambda aid=int(account["id"]): self._show_edit_account(aid)),
                (
                    "Historique",
                    lambda aid=int(account["id"]): self._open_account_history(0, 0, aid),
                ),
                ("Supprimer", lambda aid=int(account["id"]): self._delete_account(aid)),
            ]
    ...
    def _account_form(self, title: str, account=None) -> QDialog:
        ...
        balance.setSuffix(" €")
        balance.setValue(float(account["balance"]) if account else 0)
        if account:
            ...
            try:
                if account is None:
                    ...
                    self._finance.add_account(
                        ...
                        shared="Foyer" in text,
                        is_liability="Dette" in text,
    ...
    def _delete_account(self, account_id: int) -> None:
        response = QMessageBox.question(
            self,
            "Supprimer le compte",
            "Supprimer le compte et ses données associées ?",
        )
        if response != QMessageBox.StandardButton.Yes:
            return
        try:
            self._finance.delete_account(self._user.id, account_id)
    ...
    def _open_account_history(self, row=0, column=0, account_id=None) -> None:
        ...
        table = QTableWidget(len(history), 2)
        table.setHorizontalHeaderLabels(["Date", "Solde"])
        for index, item in enumerate(history):
            table.setItem(index, 0, QTableWidgetItem(str(item["date"])))
            table.setItem(index, 1, QTableWidgetItem(money(item["balance"])))
    ...
    def _show_add_transaction(self) -> None:
        ...
        amount.setSuffix(" €")
        shared = QCheckBox("Transaction du foyer")
        fields = [
            ("Compte", account),
            ("Catégorie", category),
            ("Libellé", label),
