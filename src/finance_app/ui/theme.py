...
def stylesheet(accent: str) -> str:
    ...
    QPushButton {{
        background: #202534; border: 1px solid #30364A; border-radius: 9px;
        padding: 9px 13px; text-align: left;
    }}
    QPushButton:hover {{ border-color: {accent}; }}
    QPushButton#Primary {{ background: {accent}; color: white; border: none; font-weight: 600; text-align: center; }}
    QLineEdit, QComboBox, QDoubleSpinBox, QDateEdit {{
        background: #121620; border: 1px solid #30364A; border-radius: 8px; padding: 8px;
    }}
    QTableWidget {{ background: #151923; border: 1px solid #262B3A; border-radius: 10px; gridline-color: #242A39; }}
    QHeaderView::section {{ background: #1D2230; padding: 8px; border: none; color: #AEB6C5; }}
    QProgressBar {{ background: #202534; border: none; border-radius: 6px; text-align: center; }}
