...
class FinanceService:
    ...
    def _calculate_totals(self, user_id: int) -> dict[str, Decimal]:
        ...
        gross = sum(
            (row["balance"] for row in accounts if not row["liability"]),
            Decimal("0"),
        )
        liabilities = sum(
            (abs(row["balance"]) for row in accounts if row["liability"]),
            Decimal("0"),
        )
